...
<table class="adminMain_top">
	<tbody>
		<tr>
			<td class="td01"></td>
			<td class="td02">
			<h3 class="topTitle fb f14">添加角色</h3>
			</td>
			<td class="td03"></td>
		</tr>
	</tbody>
...
<div class="adminMain_wrap">
	...
	<div class="adminContent clearfix">
		<table width="100%" class="table_bg05">
			<tr>
				<th colspan="2" class="thTitle" scope="row"><span class="cBlue f14 fb pl20">添加角色</span></th>
			</tr>
			<tr>
				<th class="thList" scope="row"><span class="red">*</span>角色名称：</th>
				<td class="tdBox">
				    <input type="hidden" name="role_menu" id="role_menu"/>
					<input type="text" name="roleName" id="roleName" value="" style="width:300px;"/>
			...
			<tr>
				...
				<td class="tdBox">
					<textarea id="roleDesc" name="roleDesc" style="width:300px;height:100px;"></textarea>
				</td>
			</tr>
			<tr>
				...
				<td class="tdBox">
					 <#if menulist??>
                            <#list menulist as menu>
                             <input type="checkbox" value="${menu.id!''}" id="menu_${menu.id}" class="menu_${menu.id}" onchange="selectMenu(this.checked,'menu_${menu.id}');"/><label>&nbsp;<font style="font-weight:bold;font-size:11pt;">${menu.menuName!''}</font>&nbsp;</label><p>
                               </br>
                              <#if menu.mlist??>
                                 <p style="margin-left:53px;">
                                   <#list menu.mlist as _menu>
					                 <input type="checkbox" value="${_menu.id!''}" class="menu_${menu.id}" onchange="selectParentMenu('menu_${menu.id}');"/><label>&nbsp;${_menu.menuName!''}&nbsp;</label>
                                   </#list>
                                 </p>
                            </#if>
                            </br>
...
<script type="text/javascript">
	...
	function checkInfo(){
	    ...
	    $.ajax({
					type : "POST",
					url : "${rc.getContextPath()}/role/save?format=json",
					data : params,
	                dataType: "text",
